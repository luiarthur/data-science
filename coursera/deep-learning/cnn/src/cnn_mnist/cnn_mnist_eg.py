...
def main(unused_argv):
    ...
    train_data = mnist.train_images().astype(np.float32)  # Returns np.array
    train_labels = mnist.train_labels().astype(np.int32)
    eval_data = mnist.test_images().astype(np.float32)  # Returns np.array
    eval_labels = mnist.test_labels().astype(np.int32)
    ...
    print("Create Training Input Fn...")
    # Train the model
    train_input_fn = tf.estimator.inputs.numpy_input_fn(
        x={"x": train_data},
        y=train_labels,
        batch_size=50, # minibatch size
        num_epochs=None, # if set steps below, then set num_epochs=None
        shuffle=True)
